...
| `reset-passwords-secure.js` | Resets passwords for configured users in the database using a supplied password value | Critical | Account takeover, uncontrolled credential reset, or inadequate reset audit | Authorized security/platform administrator only under incident or approved support procedure | No shared bulk production reset by default; no secret logging; target confirmation; secure secret input; audit event; prefer token-based application reset flow |
| `reset-passwords-simple.js` | Password-reset demonstration utility that reads environment configuration and logs sensitive credential-related data | Critical | Secret disclosure in terminal or CI logs if used with real configuration | No production use; retire or quarantine | Remove sensitive logging and insecure demonstration path in a separate security task; never expose environment values |
| `reset-passwords-standalone.js` | Standalone password reset simulation that logs database/credential-related data | Critical | Secret disclosure and operator confusion about whether changes are live | No production use; retire or quarantine | Remove sensitive output; clearly separate demonstrations from operational utilities; never run in deployment |
| `lib/db/reset-passwords.js` | Direct database password updater with embedded operational targets and credential material | Critical | Credential compromise and unauthorized account changes | No production use in current form | Quarantine immediately through a security remediation task; replace with approved reset workflow; remove hardcoded sensitive data and plaintext logging |
